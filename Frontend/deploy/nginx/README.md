...
- **کش:** پروژه برای JS/HTML هدر `Cache-Control: no-store` می‌فرستد (تا نسخهٔ جدید همیشه بیاید). این با Nginx هم محترم شمرده می‌شود؛ در عوض سرعت لود کمی پایین‌تر است. اگر خواستی، بعداً «نسخه‌دهی با `?v=`» جایگزین شود.
